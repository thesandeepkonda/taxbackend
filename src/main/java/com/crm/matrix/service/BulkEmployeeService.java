...
public class BulkEmployeeService {
    ...
    private static final String[] EXPECTED_HEADERS = {
            ...
            "Email",
            "Phone",
            "Department",           // Changed from Department ID
            "Team ID",
            "Role",                 // Changed from Role ID
    ...
    private CreateEmployeeRequest readEmployeeRow(Row row, int rowNumber, List<BulkEmployeeError> errors) {
        ...
        String departmentValue = getCellValue(row.getCell(5)); // Changed from ID
        String teamIdValue = getCellValue(row.getCell(6));
        String roleValue = getCellValue(row.getCell(7));       // Changed from ID
        ...
        if (departmentValue.isBlank()) {
            addError(errors, rowNumber, "department", "", "Department is required");
        }
        ...
        if (!departmentValue.isBlank()) {
            try {
                department = Department.valueOf(departmentValue.trim().toUpperCase());
            } catch (IllegalArgumentException e) {
                addError(errors, rowNumber, "department", departmentValue, "Invalid department. Allowed values: " + Arrays.toString(Department.values()));
            }
        }
        ...
        if (!roleValue.isBlank()) {
            try {
                ...
                addError(errors, rowNumber, "role", roleValue, "Invalid role. Allowed values: " + Arrays.toString(Role.values()));
            }
        }
        ...
        if (!workModeValue.isBlank()) {
            try {
                ...
                addError(errors, rowNumber, "workMode", workModeValue, "Invalid work mode. Allowed values: " + Arrays.toString(WorkMode.values()));
            }
        }
        ...
        request.setDepartment(department); // Setting Enum directly
        request.setTeamId(teamId);
        request.setRole(role);             // Setting Enum directly
